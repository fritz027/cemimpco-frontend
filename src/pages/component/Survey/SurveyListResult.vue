...
import { createSurvey, allSurvey, updateSurvey } from '@/services/survey.services'
...
async function loadSurveys() {
  ...
  .sort((a: any, b: any) => {
    if (a.survey_status === 'Active') return -1
    if (b.survey_status === 'Active') return 1
    return 0
  })
  .map((s: any) => ({
    id: s.survey_id,
...
async function goToResult(id: any) {
  await router.push({ name: 'SurveyQuestionResult',
    params: { id: id }
  });
}
</script>