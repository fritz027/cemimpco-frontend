  ...
  <div class="min-h-screen w-full bg-slate-50">
    <div class="w-full p-6">
      ...
      <div class="flex items-start justify-between gap-4">
        <div>
          <h1 class="text-3xl font-bold tracking-tight text-slate-900">SURVEY ELECTION</h1>
          <p class="mt-2 max-w-2xl text-sm text-slate-500">
            User access for online survey.
          </p>
        </div>
      ...
      >
        ...
        <div class="px-6 pt-6">
          <div class="flex items-start justify-between gap-3">
            <div>
              <h2 class="text-lg font-bold text-slate-900">Add Survey User</h2>
            ...
            <div class="flex gap-3">
              ...
              <div>
                ...
                <p class="mt-1 text-xs text-blue-700">
                  This updates the user list used for online survey access.
                </p>
              </div>
            </div>
          </div>
...
async function loadUsers() {
  try {
    const res = await surveyUsers(authStore.accessToken);
    if (!res.data.success) {
      console.log(res.data.message);
      return;
    }
    rows.value = (res.data.members ?? []) as SurveyUserRow[];
...
async function saveChanges() {
  ...
  try {
    const res = await addSurveyUser(memberNo, authStore.accessToken);
    if (!res.data.success) {
      console.log(res.data.message);
      return;
    }
...
async function onDelete(memberNo: string) {
  try {
    const ok = confirm("Remove this user from survey access?");
    if (!ok) return;

    deletingId.value = memberNo;

    const res = await removeSurveyUser(memberNo, authStore.accessToken);
    if (!res.data.success) {
      console.log(res.data.message);
      return;
    }
